...
export default function EmployeeMyQueries() {
  const { user } = useAuth();
  ...
  const loadQueries = async () => {
    try {
      ...
      const response = await queryAPI.getUserQueries(user!.id, filters.status || undefined);
      ...
      if (filters.queryType) {
        filteredQueries = filteredQueries.filter(
          (query: any) => query.query_type === filters.queryType
        );
      }
    ...
    <Layout theme="light">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">View and manage all your submitted queries</p>
        </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      ...
                      <th className="text-left text-gray-600 font-medium pb-4">Assigned To</th>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">
                  {queries.filter(q => q.status === 'assigned' || q.status === 'in_progress').length}
                </p>
              </div>
